...
void Player::RewardCurrency(Unit *victim)
{
    ...
    CurrencyOnKillEntry const* Cur = sObjectMgr->GetCurrencyOnKillEntry(victim->ToCreature()->GetCreatureInfo()->Entry);
...
bool Player::CanCompleteQuest(uint32 quest_id)
{
    if (quest_id)
    {
        ...
        if (q_status.m_status == QUEST_STATUS_INCOMPLETE)
        {
            ...
            if (qInfo->HasFlag(QUEST_TRILLIUMEMU_FLAGS_KILL_OR_CAST | QUEST_TRILLIUMEMU_FLAGS_SPEAKTO))
            {
            ...
            if (qInfo->HasFlag(QUEST_TRILLIUMEMU_FLAGS_PLAYER_KILL))
                if (qInfo->GetPlayersSlain() != 0 && q_status.m_playercount < qInfo->GetPlayersSlain())
                    return false;

            if (qInfo->HasFlag(QUEST_TRILLIUMEMU_FLAGS_EXPLORATION_OR_EVENT) && !q_status.m_explored)
                return false;

            if (qInfo->HasFlag(QUEST_TRILLIUMEMU_FLAGS_TIMED) && q_status.m_timer == 0)
                return false;
...
bool Player::CanCompleteRepeatableQuest(Quest const *quest)
{
    ...
    if (quest->HasFlag(QUEST_TRILLIUMEMU_FLAGS_DELIVER))
        for (uint8 i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; i++)
...
void Player::AddQuest(Quest const *quest, Object *questGiver)
{
    ...
    if (quest->HasFlag(QUEST_TRILLIUMEMU_FLAGS_KILL_OR_CAST | QUEST_TRILLIUMEMU_FLAGS_SPEAKTO))
    {
        for (uint8 i = 0; i < QUEST_OBJECTIVES_COUNT; ++i)
            questStatusData.m_creatureOrGOcount[i] = 0;
    }

    if (quest->HasFlag(QUEST_TRILLIUMEMU_FLAGS_PLAYER_KILL))
    ...
    uint32 qtime = 0;
    if (quest->HasFlag(QUEST_TRILLIUMEMU_FLAGS_TIMED))
    {
...
{
    if (!m_timedquests.empty() && qInfo->HasFlag(QUEST_TRILLIUMEMU_FLAGS_TIMED))
    {
        if (msg)
...
void Player::ItemAddedQuestCheck(uint32 entry, uint32 count)
{
    for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
        ...
        if (!qInfo || !qInfo->HasFlag(QUEST_TRILLIUMEMU_FLAGS_DELIVER))
            continue;
...
void Player::KilledMonsterCredit(uint32 entry, uint64 guid)
{
    ...
    for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
        ...
        {
            if (qInfo->HasFlag(QUEST_TRILLIUMEMU_FLAGS_KILL_OR_CAST))
            {
                for (uint8 j = 0; j < QUEST_OBJECTIVES_COUNT; ++j)
                {
    ...
    if (result)
    {
        do
        {
            ...
            if (quest)
            {
                ...
                if (quest->HasFlag(QUEST_TRILLIUMEMU_FLAGS_TIMED) && !GetQuestRewardStatus(quest_id))
                {
                    AddTimedQuest(quest_id);
    ...
    if (getLevel() <= sWorld->getIntConfig(CONFIG_MAX_RECRUIT_A_FRIEND_BONUS_PLAYER_LEVEL) || !forXP)
    {
        if (Group* group = this->GetGroup())
        {
            for (GroupReference *itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
            {
                ...
                if (!groupGuy->IsAtRecruitAFriendDistance(this))
                    continue;                               // member (alive or dead) or his corpse at req. distance

                if (forXP)
                {
                    // level must be allowed to get RaF bonus
                    if (groupGuy->getLevel() > sWorld->getIntConfig(CONFIG_MAX_RECRUIT_A_FRIEND_BONUS_PLAYER_LEVEL))
                        continue;

                    // level difference must be small enough to get RaF bonus, UNLESS we are lower level
                    if (groupGuy->getLevel() < getLevel())
                        if (uint8(getLevel() - groupGuy->getLevel()) > sWorld->getIntConfig(CONFIG_MAX_RECRUIT_A_FRIEND_BONUS_PLAYER_LEVEL_DIFFERENCE))
                            continue;
                }

                bool ARecruitedB = (groupGuy->GetSession()->GetRecruiterId() == GetSession()->GetAccountId());
                bool BRecruitedA = (GetSession()->GetRecruiterId() == groupGuy->GetSession()->GetAccountId());
...
void Player::RewardPlayerAndGroupAtEvent(uint32 creature_id, WorldObject* pRewardSource)
{
    ...
    if (Group *group = GetGroup())
    {
        for (GroupReference *itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
        {
            ...
            if (!groupGuy->IsAtGroupRewardDistance(pRewardSource))
                continue;                               // member (alive or dead) or his corpse at req. distance

            // quest objectives updated only for alive group member or dead but with not released body
            if (groupGuy->isAlive()|| !groupGuy->GetCorpse())
                groupGuy->KilledMonsterCredit(creature_id, creature_guid);
        }
    }
    else                                                    // if (!group)
...
void Player::LearnTalent(uint32 talentId, uint32 talentRank, bool one)
{
                    ...
                    if (thisrank != -1)
                    {
                        if (thisTalent->TalentTab == TalentBranchSpec(m_activeSpec))
                        {
                            ...
                            if (curtalent_maxrank != -1 && thisrank == curtalent_maxrank)
                                pointsinBranchSpec += curtalent_maxrank + 1;
                        }
                    }
                }
            }
        }
        if (pointsinBranchSpec < 31)
            return;
    }
    ...
    {
        if (TalentEntry const* TalentInfo = sTalentStore.LookupEntry(talentInfo->DependsOn))
        {
...
{
    //              0             1
    // SetPQuery(CHAR_LOAD_PLAYER_TALENTBRANCHSPECS, "SELECT talentbranchSpec, spec FROM character_talentbranchspec WHERE guid = '%u'", GUID_LOPART(m_guid));
    if (result)
    {
        do
        {
