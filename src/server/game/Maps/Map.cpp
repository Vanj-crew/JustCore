...
{
    CellCoord p = Trillium::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
    if (!p.IsCoordValid())
    {
...
void Map::EnsureGridCreated(const GridCoord &p)
{
    ...
    {
        TRILLIUMEMU_GUARD(ACE_Thread_Mutex, Lock);
        if (!getNGrid(p.x_coord, p.y_coord))
        {
...
{
    CellCoord cellCoord = Trillium::ComputeCellCoord(player->GetPositionX(), player->GetPositionY());
...
void Map::AddToMap(T *obj)
{
    ...
    CellCoord cellCoord = Trillium::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
    if (!cellCoord.IsCoordValid())
    {
...
void Map::MoveAllCreaturesInMoveList()
{
    ...
    for (std::vector<Creature*>::iterator itr = _creaturesToMove.begin(); itr != _creaturesToMove.end(); ++itr)
    {
        ...
        if (CreatureCellRelocation(c, Cell(Trillium::ComputeCellCoord(c->_newPosition.m_positionX, c->_newPosition.m_positionY))))
        {
            // update pos
...
void Map::AddToActive(Creature* c)
{
    ...
    if (!c->isPet() && c->GetDBTableGUIDLow())
    {
        ...
        GridCoord p = Trillium::ComputeGridCoord(x, y);
        if (getNGrid(p.x_coord, p.y_coord))
            getNGrid(p.x_coord, p.y_coord)->incUnloadActiveLock();
        else
        {
            GridCoord p2 = Trillium::ComputeGridCoord(c->GetPositionX(), c->GetPositionY());
